        ...
        if (task.startTime !== 0) {
            ...
            if (task) {
                const _changedTask = {
                    ...
                    projectId: task.projectId,
                    time: task.time,
                    tracking: Number((task.tracking + totalTrackedTimeMin + totalTrackedTimeSec2).toFixed(2)),
                    complete: task.complete
                }
        ...
        activeTaskId && task ?
            <div className="tracker">
                            ...
                            <>
                                {
                                    task.tracking >= task.time ?
                                        <>
                                            Трекинг → [{formatTime(0)} - <span className='tracker__overtime'>overtime</span>]
                                        </>
                                        :
                                        <>
                                            Трекинг → [{formatTime(0)} - {formatTime(task.time - Math.floor(task.tracking))}]
                                        </>
                                }
                            </>
                            :
                ...
                <div className="tracker__control">
                    <Button classes={'tracker__button'} disabled={task.startTime > 0 || task.complete} text='Начать трекинг' onClick={handleStartTracking}/>
                    <Button classes={'tracker__button'} disabled={task.startTime === 0 || task.complete} text='Пауза' onClick={handleStopTracking}/>
                </div>
                <div className="tracker__control">
                    <Button classes={'tracker__button'} disabled={task.complete} text='Выполнить' onClick={complete}/>
                    <Button classes={'tracker__button'} disabled={!task.complete} text='Сводка' onClick={()=>setModal(true)}/>
                </div>
            </div>
            :
            null
