...
const ProjectCard: React.FC<projectsTypes> = (props: projectsTypes) => {
    ...
    const completedTasks = tasks.filter((task) => task.projectId === props.id && task.complete).length;
    const procents = completedTasks && (completedTasks / tasks.length * 100).toFixed(2)
    ...
    return (
        <NavLink to={'/main/project/' + props.id} className="projectCard">
            <div className="projectCard__content">
                ...
                <p style={{color: `var(--${color})`}} className='projectCard__description'>
